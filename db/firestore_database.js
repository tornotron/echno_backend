  ...
  try {
    ...
    for (const doc of snapshot.docs) {
      ...
      if (subsnapshot.empty) {
        console.log(
          `No documents found in subcollection 'machines' for document ${doc.id}`,
        );
        return;
      } else {
...
async function inventoryRequestForward(requestId, location) {
  try {
    ...
    await inventoryRequestForwarddb
      .doc(requestId)
      .set({ status: "forwarded", location: location }, { merge: true });
  } catch (error) {
...
async function storeResponse(requestId, availableItems) {
  try {
    ...
    await storeResponsedb
      .doc(requestId)
      .set(
        { status: "completed", availableItems: availableItems },
        { merge: true },
      );
  } catch (error) {
...
async function inventoryItemDelete(location, item) {
  try {
    ...
    const snapshot = await inventoryDeleteRef
      .doc(location)
      .collection("machines")
      .doc(item)
      .get();
    ...
    const result = await inventoryDeleteRef
      .doc(location)
      .collection("machines")
      .doc(item)
      .delete();
