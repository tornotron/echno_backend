...
async function PinventoryCreate(item, location, itemJSON) {
  try {
    ...
    await pool.query(
      `INSERT INTO inventory (item_id, item_name, location, statusOfItem) VALUES ($1, $2, $3, $4)`,
      [item_id, item, location, itemJSON.statusOfItem],
    );
  } catch (error) {
...
async function PspecificInventoryFetch(filteredParams) {
  try {
    if (
      filteredParams.item_name &&
      filteredParams.item_id &&
      filteredParams.location
    ) {
      const result = await pool.query(
        `SELECT * FROM inventory WHERE item_name = $1 AND item_id = $2 AND location = $3`,
        [
          filteredParams.item_name,
          filteredParams.item_id,
          filteredParams.location,
    ...
    if (filteredParams.location) {
      const result = await pool.query(
        `SELECT * FROM inventory WHERE location = $1`,
        [filteredParams.location],
      );
      return result.rows;
    }
    ...
    if (filteredParams.statusOfItem) {
      const result = await pool.query(
        `SELECT * FROM inventory WHERE statusOfItem = $1`,
        [filteredParams.statusOfItem],
      );
      return result.rows;
    }
...
async function PinventoryRequestCreate(newrequest) {
  try {
    await pool.query(
      `INSERT INTO inventoryRequests (request_id, requested_items, status) VALUES ($1, $2, $3)`,
      [newrequest.requestId, newrequest.requestedItems, newrequest.status],
    );
  } catch (error) {
...
async function PinventoryRequestForward(requestId, location) {
  try {
    await pool.query(
      `UPDATE inventoryRequests SET status = $1, location = $2 WHERE request_id = $3`,
      ["forwarded", location, requestId],
    );
  } catch (error) {
...
async function PspecificInventoryRequestfetch(filteredParams) {
  try {
    if (filteredParams.requestId) {
      const result = await pool.query(
        `SELECT * FROM inventoryRequests WHERE request_id = $1`,
        [filteredParams.requestId],
      );
      return result.rows;
    }
  } catch (error) {
...
async function PstoreResponse(requestID, availableItems) {
  try {
    const result = await pool.query(
      `UPDATE inventoryRequests SET status = $1, available_items = $2 WHERE request_id = $3`,
      ["completed", availableItems, requestID],
    );
    result.rows;
  } catch (error) {
